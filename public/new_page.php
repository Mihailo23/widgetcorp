...
<?php
	// can't add new page unless we have a subject as a parent
    if (!$current_subject) {
...
<?php 
	if (isset($_POST["submit"])) {
		...
		$fields_with_max_lenghts = array("menu_name" => 30, "content" => 200);
		...
		<form action="new_page.php?subject=<?php echo urlencode($current_subject["id"]); ?>" method="post">
			...
			<p>Position:
				...
				<?php 
					...
					for ($count=1; $count <= $page_count + 1; $count++) { // +1 is because we need to have an option to put the subject in the last place (to increment)
